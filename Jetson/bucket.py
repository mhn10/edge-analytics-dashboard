...
class Jetson:
    ...
    def CollectFiles( self ):
        ...
        if not os.path.exists( self.__userName + "/Results/" ):
            os.makedirs( '{0}/Results/'.format( self.__userName ) )
    ...
    def CreateVirtualEnv( self ):
        ...
        # print( self.__envName )
        # print( "{0}/bin/activate".format(self.__envName))
        commands = { 'create': 'python3 -m venv {0} --system-site-packages'.format( self.__envName ),
        ...
        print( commands['activate'] )
        # Create virtual env
        # subprocess.call( commands['create'], shell=True )
        # subprocess.call( commands['activate'], shell = True )
        # Run all commands in virtual env
        # myProcess = subprocess.Popen( '{0}; {1}; {2}; {3}; {4}; {5}'.format( commands['activate'], commands['install'], commands['changeDir'], commands['run'], commands['backToDir'], commands['deactivate'] ), shell=True, stdout = subprocess.PIPE, stderr=subprocess.PIPE )
        # myProcess = subprocess.call( 'ls; {0}; {1}; {2}; {3}; {4}; {5}'.format( commands['activate'], commands['install'], commands['changeDir'], commands['run'], commands['backToDir'], commands['deactivate'] ), shell=True )
